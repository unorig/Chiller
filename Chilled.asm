...
Mod_LoadCharSet                                 ; This will modify the code for LoadCharSet based on the passed X and Y values.
        stx     Low_tempvar 			; Set Low_tempvar to X
        sty     High_tempvar 			; Set High_tempvar to Y
        ldy     #$00       			; Set Y to #00
        lda     (Low_tempvar),y 		; A = 5bbf,00. Start of the index set.
        sta     LoadCharSet+1 			; $C29E = #00
        iny                			; Increase Y
        lda     (Low_tempvar),y 		; A = #b2
        sta     LoadCharSetMod1+1    		; $C2A2 = #b2
        iny                			; Increase Y
        lda     (Low_tempvar),y 		; A = #ff
        sta     LoadCharSetMod4+1    		; $C2AE = #ff
        iny                			; Increase Y
        lda     (Low_tempvar),y 		; A = #b5
        sta     LoadCharSetMod5+1    		; $C2B3 = #b5
        iny                			; Increase Y
        lda     (Low_tempvar),y 		; A = 00
        sta     LoadCharSetMod2+1    		; $C2A6 = #00
        iny                			; Increase Y
        lda     (Low_tempvar),y 		; A = #30
        sta     LoadCharSetMod3+1    		; C2AA = #30
        jsr     LoadCharSet                     ; Jump to the routine to load the charset.
...
L5BC7   ldx     #$bf
        ldy     #$5b
        jsr     Mod_LoadCharSet   		; JSR $2a80 with Y = #5b / X = #bf
        lda     #$00				; A = #00
...
Sub_ResetToMenu
	...
	sta     L771D                 	        ; Store accumulator value into L771D
        ...
        ; Load specific values into X and Y registers and call Mod_LoadCharSet
        ldx     #$bf                            ; Load #$bf into the X register
        ldy     #$5b                            ; Load #$5b into the Y register
        jsr     Mod_LoadCharSet                 ; Call subroutine Mod_LoadCharSet
...
JUMP_c646
        ...
        ldx     #$c0                            ; Set X to #c0
        ldy     #$56                            ; Set Y to #56
        jsr     Mod_LoadCharSet                 ; Jump to subroutine at Mod_LoadCharSet
        nop					; No operation.
        nop					; No operation.
        nop					; No operation.
        nop					; No operation.
...
LC71B   lda     #$00                		; A = #00
        ...
        jsr     Sub_GetInputs 			; Get keyboard and joystick inputs and store to variables.
...
_rts    rts               			; Return from the subroutine.
        ...
        .byte   $ea,$ea,$ea,$ad,$3c,$03,$ae,$3d,$03,$20,$00,$c9,$60 ;Undocumented instructions.
                                                ...
                                                ;* Get Inputs                *
                                                ;*****************************
Sub_GetInputs                                   ; Get keyboard and joystick inputs and store to variables.
        lda     #$00                    	; A = #00
...
LCC1F   ldx     Temp_CurrentEnemy               ; Set X to Temp_CurrentEnemy
        inc     LCF1A,x                         ; Increase CF1A,x (x = Temp_CurrentEnemy)
        lda     LCF1A,x                         ; Set A to CF1A,x (x = Temp_CurrentEnemy)
        cmp     #$08                            ; Compare A to #08
        beq     If_CC2D                         ; Branch if A = #08. This will then reset the counter to #00.
        rts

If_CC2D lda     #$00                            ; Set A to #00
        sta     LCF1A,x                         ; Reset LCF1A,x to #00
        lda     L453D,x
        cmp     #$00
        beq     If_CC3C
        jmp     Jump_CC65
